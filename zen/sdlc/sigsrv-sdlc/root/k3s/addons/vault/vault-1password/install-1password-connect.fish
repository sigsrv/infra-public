...
set --unpath OP_1PASSWORD_CREDENTIALS_PATH "op://sigsrv-sdlc/sigsrv-sdlc-vault-op-file/1password-credentials.json"
...
# setup cerd
op read $OP_1PASSWORD_CREDENTIALS_PATH | base64 | tr -d '\n' > /tmp/1password-credentials.json
vault kv put -cas 0 secret/addon/1password/op-credentials 1password-credentials.json=@/tmp/1password-credentials.json
